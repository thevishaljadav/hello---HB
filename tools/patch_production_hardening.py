...
if "proguardFiles getDefaultProguardFile('proguard-android-optimize.txt'), 'proguard-rules.pro'" not in s:
...
h = html.read_text()
h = re.sub(r'https://cdn\.jsdelivr\.net/npm/@supabase/supabase-js@[^"\']+', 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.116.0', h, count=1)
...
j = java.read_text()
# Idempotently replace the old debug-only guard, including versions already partially patched.
j = re.sub(r'if \(BuildConfig\.DEBUG\) WebView\.setWebContentsDebuggingEnabled\(true\);', 'if ((getApplicationInfo().flags & android.content.pm.ApplicationInfo.FLAG_DEBUGGABLE) != 0) {\n            WebView.setWebContentsDebuggingEnabled(true);\n        }', j)
