...
public class MainActivity2 extends Activity {
        ...
        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && purchases != null) {
            for (Purchase purchase : purchases) {
                for (String productId : purchase.getProducts()) {
                    ...
                    runJs("window.onPlayPurchase && window.onPlayPurchase(" + js(productId) + "," + js(token) + ")");
                }
            }
    ...
    private void buy(String productId) {
        if (billingClient == null || !billingClient.isReady()) {
            Toast.makeText(this, "Google Play is connecting. Try again.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        billingClient.queryProductDetailsAsync(params, (result, details) -> {
            if (result.getResponseCode() != BillingClient.BillingResponseCode.OK || details.getProductDetailsList().isEmpty()) {
                runJs("window.onPlayPurchaseError && window.onPlayPurchaseError('Product is not available on Google Play yet.')");
                return;
            }
    ...
    @Override public void onBackPressed() {
        if (web != null) {
            boolean handled = Boolean.TRUE.equals(web.evaluateJavascript("window.handleAndroidBack ? window.handleAndroidBack() : false", null));
