...
public class SupabaseApi {
  ...
  public synchronized boolean saveProfile(String name,String mode,int day,String bio){try{request("POST","/rest/v1/profiles?on_conflict=id",new JSONObject().put("id",userId).put("full_name",name).put("mode",mode).put("navratri_day",day).put("bio",bio).put("updated_at",new java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").format(new java.util.Date())).toString(),token);return true;}catch(Exception e){error=e.getMessage();return false;}}
